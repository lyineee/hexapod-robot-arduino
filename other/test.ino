...
void loop()
{
  ...
  // position(&straight_ahead_right,&straight_middle_right,&straight_back_right,50,25,1);
  // Serial.println("ok");
  ...
  // {
    // walk(state, stage, 25);
    walk(20, stage, 30,3,30,3);  //4.88 delta=283
    // walk(15, stage, 30,3,30,3);  // 5.10
    // right(10, stage, 30,1,30,2);
    // right_2(10, stage, 30,1,30,2);
    if (stage == 1)
      stage = 2;
    else
      stage = 1;
  ...
  //8
  //9
  //12: m 193 - 70 - max 50 middle :285
  //13
  //14:max 470 - 75
}

void test_speed(uint16_t delay_time){
  pwm_0.setPWM(5,0,92);
  delay(delay_time);
  // pwm_0.setPWM(5,0,200);
  pwm_0.setPWM(5,0,300);
  delay(delay_time);
  // Serial.println('a');
}
...
//   Serial.println(a);
//   // Serial.println(digitalRead(change_btn));
...
void set_servo_position_test()
{
  ...
  for (uint16_t i = 0; i < sample_num; i += 1)
  {
    ...
    pwm_1.setPWM(8, 0, pgm_read_word(&straight_middle_left[i][0]) + 70);
    ...
    pwm_0.setPWM(14, 0, pgm_read_word(&straight_ahead_right[i][2]));
    // //backward
    pwm_1.setPWM(4, 0, pgm_read_word(&straight_ahead_left[sample_num - i][0]));
    pwm_1.setPWM(5, 0, pgm_read_word(&straight_ahead_left[sample_num - i][1]) - hang);
    pwm_1.setPWM(6, 0, pgm_read_word(&straight_ahead_left[sample_num - i][2]));

    pwm_0.setPWM(9, 0, pgm_read_word(&straight_middle_right[sample_num - i][0]));
    pwm_0.setPWM(10, 0, pgm_read_word(&straight_middle_right[sample_num - i][1]) - hang);
    pwm_0.setPWM(11, 0, pgm_read_word(&straight_middle_right[sample_num - i][2]));

    pwm_1.setPWM(12, 0, pgm_read_word(&straight_back_left[sample_num - i][0]));
    pwm_1.setPWM(13, 0, pgm_read_word(&straight_back_left[sample_num - i][1]) - hang);
    pwm_1.setPWM(14, 0, pgm_read_word(&straight_back_left[sample_num - i][2]));
    delay(stop);
  }
  delay(100);
  ...
  for (uint16_t i = 0; i < sample_num; i += 1)
  {
    ...
    pwm_1.setPWM(14, 0, pgm_read_word(&straight_back_left[i][2]));
    //backward
    pwm_0.setPWM(5, 0, pgm_read_word(&straight_back_right[sample_num - i][0]));
    pwm_0.setPWM(6, 0, pgm_read_word(&straight_back_right[sample_num - i][1]) - hang);
    pwm_0.setPWM(7, 0, pgm_read_word(&straight_back_right[sample_num - i][2]));

    pwm_1.setPWM(8, 0, pgm_read_word(&straight_middle_left[sample_num - i][0]) + 70);
    pwm_1.setPWM(9, 0, pgm_read_word(&straight_middle_left[sample_num - i][1]) - hang);
    pwm_1.setPWM(10, 0, pgm_read_word(&straight_middle_left[sample_num - i][2]));

    pwm_0.setPWM(12, 0, pgm_read_word(&straight_ahead_right[sample_num - i][0]));
    pwm_0.setPWM(13, 0, pgm_read_word(&straight_ahead_right[sample_num - i][1]) - hang);
    pwm_0.setPWM(14, 0, pgm_read_word(&straight_ahead_right[sample_num - i][2]));
    delay(stop);
  }
  delay(100);
}
